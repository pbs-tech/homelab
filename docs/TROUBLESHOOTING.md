...
#### 6. No Peers / Wrong Public Key

**Symptoms:**

- `wg show` displays the interface but no `[Peer]` section
- Interface comes up but no traffic flows (ping to `10.200.0.1` fails)
- `wg show` reports `latest handshake: never`

**Root Cause:**

The client's `wg0.conf` has the client's own public key in the `[Peer]` section instead of
the **server's** public key. WireGuard finds no matching peer and the tunnel carries no traffic.

**Diagnostics:**

```bash
# On the client — check what public key is configured in [Peer]
grep PublicKey /etc/wireguard/wg0.conf

# On the Proxmox host — get the actual server public key
pct exec 203 -- wg show wg0 public-key
# Or read the file saved by Ansible:
pct exec 203 -- cat /etc/wireguard/server_publickey
```

If the two values match, the config is wrong (client key used as peer key).

**Fix:**

1. Replace the `PublicKey` value under `[Peer]` in `/etc/wireguard/wg0.conf` with the
   server's public key obtained above.
2. Alternatively, use the Ansible-generated config (`~/.wireguard/homelab/<name>-wg0.conf`)
   which already has the correct server public key pre-populated.
3. Restart the tunnel: `sudo wg-quick down wg0 && sudo wg-quick up wg0`

#### 5. `wg-quick` Fails on Client Machine (`resolvconf` Signature Mismatch)

**Symptoms:**

- `wg-quick up wg0` brings the interface up then immediately tears it down
- Error: `resolvconf: signature mismatch: /etc/resolv.conf`
- `wg show` shows no active interface after running `wg-quick up`

**Root Cause:**

`/etc/resolv.conf` was modified outside of `resolvconf`'s management (e.g., by NetworkManager,
systemd-resolved, or a manual edit). When `wg-quick` tries to configure DNS via `resolvconf`,
it detects the checksum mismatch and aborts, rolling back the entire interface.

**Solutions (in order of preference):**

If the WireGuard config was generated by NetworkManager, import it as an NM connection and
use `nmcli` instead of `wg-quick` — NM handles DNS itself and avoids the conflict entirely:
...
If you prefer to keep using `wg-quick`, update the resolvconf database first:
...
If the system uses `systemd-resolved`, ensure `/etc/resolv.conf` is the correct symlink:
...
If you don't need VPN-side DNS resolution, remove the `DNS =` line from
`/etc/wireguard/wg0.conf`. The tunnel will still work; only DNS routing via the VPN is skipped.
...
Use this section when the control machine has been rebuilt or migrated and SSH keys no longer
match what is installed on the infrastructure hosts.

### Raspberry Pi K3s Nodes (SD Card Method)

When SSH access is lost and password authentication is disabled, the SD cards must be mounted
directly on the control machine to restore access.

A recovery script is provided at `scripts/recovery.sh` that automates this process:
...
1. Verify your public key exists at `~/.ssh/homelab_ed25519.pub`
2. Guide you through inserting each SD card one at a time
3. Mount the root partition, add the key, and fix permissions
4. Prompt you to swap to the next card
...
# Mount root partition (adjust sdb2 to match your device)
sudo mkdir -p /mnt/pi
sudo mount /dev/sdb2 /mnt/pi

# Check pbs user UID
grep "^pbs:" /mnt/pi/etc/passwd

# Add public key
sudo mkdir -p /mnt/pi/home/pbs/.ssh
cat ~/.ssh/homelab_ed25519.pub | sudo tee -a /mnt/pi/home/pbs/.ssh/authorized_keys

# Fix permissions (critical - wrong perms silently block SSH)
sudo chown -R 1000:1000 /mnt/pi/home/pbs/.ssh
sudo chmod 700 /mnt/pi/home/pbs/.ssh
sudo chmod 600 /mnt/pi/home/pbs/.ssh/authorized_keys

sudo umount /mnt/pi
```

Repeat for all four Pi nodes (`k3-01` through `k3-04`). Process `k3-01` (server node at
`192.168.0.111`) first to verify the fix before doing the agents.

**Verify after reinserting all SD cards:**

```bash
ansible k3s_cluster -m ping -i inventory/hosts.yml
```

### Proxmox Hosts (Web Console Method)

Proxmox VE provides a browser-based shell that does not require SSH, making it the primary
recovery path for `pve-mac` and `pve-nas`.

**Step 1: Add key via web console**

1. Open `https://192.168.0.56:8006` (pve-mac) or `https://192.168.0.57:8006` (pve-nas)
2. Select the node in the left panel
3. Click **Shell** in the top-right toolbar
...
Get your public key contents on your control machine with:

```bash
cat ~/.ssh/homelab_ed25519.pub
```

**Step 2: Use the bootstrap playbook (alternative — requires password auth)**

If root password SSH auth is still enabled on Proxmox (it is by default), the bootstrap
playbook can handle key installation without web console access:

```bash
ansible-playbook playbooks/bootstrap-proxmox.yml --ask-pass
```

**Step 3: Restore LXC container access via `pct exec`**

Once you have SSH or console access to a Proxmox host, push the new key into all running
containers without requiring SSH to each one:

```bash
# Run on the Proxmox host — pushes key to every running container
PUBKEY="$(cat ~/.ssh/homelab_ed25519.pub)"
for ct in $(pct list | awk 'NR>1 && $2=="running" {print $1}'); do
  echo "Pushing key to CT $ct..."
  pct exec $ct -- bash -c \
...
```bash
pct exec 200 -- bash -c "echo 'your-key' >> /root/.ssh/authorized_keys"
```

**Verify all hosts are reachable after recovery:**

```bash
ansible all -m ping -i inventory/hosts.yml --vault-password-file ~/.ansible_vault_pass
```
